...
/**
 * Class for constraction of WURCS2.0 RDF (ver 0.5.1) model using jena API
 * @author MasaakiMatsubara
 *
 */
public class WURCSRDFModelGlycanOld extends WURCSRDFModel{

	@SuppressWarnings("unused")
	private WURCSRDFModelGlycanOld(){}

	public WURCSRDFModelGlycanOld(String a_strAccessionNumber, WURCSArray a_oWURCS){
		super();
		createWURCSGlycanTripleModel(a_strAccessionNumber, a_oWURCS);
	}

	public void addWURCSGlycanTripleModel(String a_strAccessionNumber, WURCSArray a_oWURCS) {
		createWURCSGlycanTripleModel(a_strAccessionNumber, a_oWURCS);
	}

	private WURCSRDFModelGlycanOld createWURCSGlycanTripleModel(String a_strAccessionNumber, WURCSArray a_oWURCS){
		...
		WURCSRDFModelGlycanOld model = this;
		...
		model.addTriple(createResourceTriple(
				WURCSTripleURL_TBD.GSEQ_AC.get(a_strAccessionNumber, null),
				WURCS.HAS_ROOT_RES,
				WURCSTripleURL_TBD.RES_AC.get(a_strAccessionNumber, "a")));
		...
		for (UniqueRES t_oURES : a_oWURCS.getUniqueRESs()) {
			...
			model.addTriple(createResourceTriple(
					WURCSTripleURL_TBD.GSEQ_AC.get(a_strAccessionNumber, null),
					WURCS.HAS_URES,
					WURCSTripleURL_TBD.UNIQ_RES.get(a_strAccessionNumber, t_oURES.getUniqueRESID())));

			// For monosaccharide of unique RES
			model.addTriple(createResourceTriple(
					WURCSTripleURL_TBD.GSEQ_AC.get(a_strAccessionNumber, null),
					WURCS.HAS_MS,
					WURCSTripleURL_TBD.MS.get(a_strAccessionNumber, export.getUniqueRESString(t_oURES))));
			...
			model.addTriple(createResourceTriple(
					WURCSTripleURL_TBD.GSEQ_AC.get(a_strAccessionNumber, null),
					WURCS.HAS_BASETYPE,
					WURCSTripleURL_TBD.BASE_TYPE.get(a_strAccessionNumber, export.getUniqueRESString(t_oBasetype))));
		}
		...
		for ( LIN t_oLIN :a_oWURCS.getLINs() ) {
			...
			model.addTriple(createResourceTriple(
					WURCSTripleURL_TBD.GSEQ_AC.get(a_strAccessionNumber, null),
					WURCS.HAS_LIN,
					WURCSTripleURL_TBD.LIN_AC.get(a_strAccessionNumber, export.getLINString(t_oLIN))));
		}
		...
		model.addTriple(createResourceTriple(
				...
				WURCSTripleURL_TBD.WURCS_URL.get(a_strAccessionNumber,  export.getWURCSString(a_oWURCS) )));
		...
		for ( RES t_oRES : a_oWURCS.getRESs() ) {
			model.addTriple(createResourceTriple(
					WURCSTripleURL_TBD.RES_AC.get(a_strAccessionNumber, t_oRES.getRESIndex()),
					RDF.TYPE.stringValue(),
					WURCS.A_RES));
			...
			model.addTriple(createResourceTriple(
					WURCSTripleURL_TBD.RES_AC.get(a_strAccessionNumber, t_oRES.getRESIndex()),
					WURCS.IS_URES,
					WURCSTripleURL_TBD.UNIQ_RES.get(a_strAccessionNumber, t_oRES.getUniqueRESID())));
		...
		for ( LIN t_oLIN : a_oWURCS.getLINs() ) {
			...
			for ( GLIPs t_oGLIPs : t_oLIN.getListOfGLIPs() ) {
				...
				model.addTriple(createResourceTriple(
						WURCSTripleURL_TBD.LIN_AC.get(a_strAccessionNumber, export.getLINString(t_oLIN)),
						WURCS.HAS_GLIPS,
						WURCSTripleURL_TBD.GLIPS.get(a_strAccessionNumber, export.getGLIPsString(t_oGLIPs))));
			}
		}
		...
		for ( GLIPs t_oGLIPs : t_aGLIPs ) {
			...
			for ( GLIP t_oGLIP : t_oGLIPs.getGLIPs() ) {
				...
				model.addTriple(createResourceTriple(
						WURCSTripleURL_TBD.GLIPS.get(a_strAccessionNumber, export.getGLIPsString(t_oGLIPs)),
						WURCS.HAS_GLIP,
						WURCSTripleURL_TBD.GLIP.get(a_strAccessionNumber, export.getGLIPString(t_oGLIP))));
			}
		}
		...
		for ( GLIP t_oGLIP : t_aGLIP ) {
			...
			model.addTriple(createResourceTriple(
					WURCSTripleURL_TBD.GLIP.get(a_strAccessionNumber, export.getGLIPString(t_oGLIP)),
					WURCS.HAS_RES,
					WURCSTripleURL_TBD.RES.get(a_strAccessionNumber, t_oGLIP.getRESIndex())));
