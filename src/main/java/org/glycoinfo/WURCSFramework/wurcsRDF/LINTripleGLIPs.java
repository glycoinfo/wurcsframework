 ...
 */
	...
	//TODO: 
	public LINTripleGLIPs(String a_strAccessionNumber, LinkedList<LIN> a_aLINs) {
...
*/			
			...
			for (GLIPs glips : lin.getListOfGLIPs()) {
				m_obj.add(new String[] {"has_GLIPS", "wurcs:has_GLIPS\t<" + URI.getGlycoInfoGlycanURI() + a_strAccessionNumber + "/wurcs/2.0/GLIPS/" + WURCSStringUtils.getURLString(export.getGLIPsString(glips)) + ">"});
			}
			this.m_objects.add(m_obj);
...
//					t_sb.append("\t" + obj[1] + " " + endString + "\n");
//				}
				else if (obj[0].equals("has_GLIPS")) {
					t_sb.append("\t" + obj[1] + " " + endString + "\n");
				}
				i++;
			}
