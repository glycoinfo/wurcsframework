...
*/
	...
	public String getSPARQL(String  a_strWURCS, LinkedList<String> t_aOption){
		...
		m_strHED += "# WURCSrdfSPARQLGLIPS_SSM\n";
		m_strHED += "# ******************************************************\n\n";
...
//		sb.append("# LIN1\n");
		...
		for (LIN a_oLIN : m_oWURCSArray.getLINs()){
			...
			for (GLIPs a_oGLIPS : a_oLIN.getListOfGLIPs()) {
				...
				// FILTER (?GLIPa3a6 = ?GLIPa3 || ?GLIPa3a6 = ?GLIPa6 )
				...
				for (GLIP a_oGLIP : a_oGLIPS.getGLIPs()){
					...
					sb.append(" ?GLIP" + WURCSSPARQLUtils_TBD.removeChar4SPARQL(export.getGLIPsString(a_oGLIPS)) + " = ?GLIP" + WURCSSPARQLUtils_TBD.removeChar4SPARQL(getGLIPSting(a_oGLIP)) + endString);
					i_GLIPS++;
				}
				sb.append("  ) \n");
...
/*				
				MINUS {
				       ?gseq wurcs:has_LIN ?LINm1 .
				       ?LINm1 wurcs:has_GLIPS ?GLIPSm1 . 

				       ?GLIPSm1 wurcs:isFuzzy "true"^^xsd:boolean .
*/
