...
//		String output = t_strDir + "all_wurcs_180615_hyphen_out_inv.txt";
...
*/
				...
				if ( ! t_strOrigWURCS.equals(t_strSortLIN) ) System.out.print(" LIN or MOD Sort :");
				if ( t_oNorm.isInverted() )                  System.out.print(" Invert :");
				if ( t_oNorm.linkedAnomericPositions() )     System.out.print(" Anomeric linkage :");
				if ( t_oNorm.hasCyclic() )                   System.out.print(" Cyclic :");
				System.out.println("\n\t"+t_strOrigWURCS);
				if ( ! t_strOrigWURCS.equals(t_strSortLIN) )
					System.out.println("\t"+t_strSortLIN);
				if ( t_strSortLIN.equals(t_strSortGraph) ) continue;
				System.out.println("\t"+t_strSortGraph);
			...
			System.out.println("Sort change count: "+t_aSortLIN.size());
			System.out.println( join(t_aSortLIN) );
			System.out.println("Invert count: "+t_aInvert.size());
			System.out.println( join(t_aInvert) );
			System.out.println("Anomeric bonding count: "+t_aAnomLink.size());
			System.out.println( join(t_aAnomLink) );
			System.out.println("Cyclic count: "+t_aCyclic.size());
			System.out.println( join(t_aCyclic) );
			System.out.println("Other count: "+t_aOthers.size());
			System.out.println( join(t_aOthers) );
			// dup
			System.out.println("Duplicate:");
			for ( String t_strWURCS : t_mapUniqueGraphWURCStoID.keySet() ) {
				if ( t_mapUniqueGraphWURCStoID.get(t_strWURCS).size() < 2 ) continue;
				System.out.println( join(t_mapUniqueGraphWURCStoID.get(t_strWURCS)) );
